...
namespace Acroname {
    namespace BrainStem {
        ...
        public:
            ...
            /// \return Returns \ref EntityReturnValues "common entity" return values
            DEPRECATED aErr setVoltage(const int32_t microvolts);
            ...
            /// \param milliwatts The power in milli-watts (1 == 1e-3W).
            /// \return Returns \ref EntityReturnValues "common entity" return values
...
//            aErr getRange(uint8_t* min, uint8_t* max, uint8_t* trim);
        ...
        public:
            ...
            //Maximum power all ports can draw.
            aErr getSystemPowerLimit(uint32_t* power);
            aErr setSystemPowerLimit(const uint32_t power);

            //Which power source is preferred.  (auto, external, control, up0, up1)
            aErr getSystemPowerPreference(uint8_t* preference);
            aErr setSystemPowerPreference(const uint8_t preference);

            aErr getSystemPowerProfile(const uint8_t index, uint32_t* profile);
            aErr setSystemPowerProfile(const uint8_t index, const uint32_t profile);
